...
public class UserLogin {
	...
	public boolean login() {
       ...
       //While Loop Exists until the user entered email is not correct 
		while (validateEmail(email)) {
			...
			case "1":
				...
				email = sc.nextLine();
				break;
				//for a new account Creation using UserSignUp Object
			case "2":
				signUpObj.storeData();
				return false;
			default:
  ...
  //checking for email existence
	public static boolean validateEmail(String email) {
		return findUser(email).isEmpty(); //returns true if there is no such email exits
	}
	 //passing the email and password arguments to findUser Method which returns a map object
